...
export async function getSystemAlerts(): Promise<AlertItem[]> {
    try {
        ...
        const { data: lowMarginQuotations } = await supabase
            .from('quotations')
            .select('id, quotation_number, margin_percent')
            .eq('status', 'sent')
        ...
        if (lowMarginQuotations && lowMarginQuotations.length > 0) {
            alerts.push({
                ...
                message: lowMarginQuotations[0].quotation_number
                    ? `${lowMarginQuotations[0].quotation_number} chỉ có margin ${lowMarginQuotations[0].margin_percent || 0}%`
                    : `Margin dưới 15%`,
                severity: 'info',
