...
import { Phone, Mail, FileText, ExternalLink, CreditCard, Save, Loader2, CheckCircle2 } from 'lucide-react'
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label className="text-xs text-muted-foreground flex items-center gap-2">
                                    ...
                                    {order.demo_link && (
                                        <a href={order.demo_link} target="_blank" className="text-primary hover:underline flex items-center gap-1 font-normal">
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs text-muted-foreground flex items-center gap-2">
                                    ...
                                    {order.resource_link && (
                                        <a href={order.resource_link} target="_blank" className="text-primary hover:underline flex items-center gap-1 font-normal">
                                            Mở link <ExternalLink className="h-3 w-3" />
                                        </a>
                                    )}
                                </Label>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-sm font-medium">Chi tiết khách hàng</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <Separator />
                            <div className="space-y-2">
                                <p className="text-xs text-muted-foreground">Ghi chú nghiệp vụ</p>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-sm font-medium">Thông tin VAT</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {order.needs_vat ? (
                                <div className="space-y-3">
                                    <Badge variant="secondary">Yêu cầu xuất VAT</Badge>
                                    ...
                                    <div className="space-y-1 text-sm">
                                        ...
                                        <p className="text-sm text-muted-foreground">{order.vat_info?.company_address || '---'}</p>
                                    </div>
                                </div>
                            ) : (
            ...
            <div className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-1">
                            ...
                            <p className="text-2xl font-semibold tabular-nums">{formatCurrency(order.total_amount)}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <p className="text-xs text-muted-foreground">Đã thu</p>
                                <p className="text-base font-medium tabular-nums">{formatCurrency(order.paid_amount || 0)}</p>
                            </div>
                            <div className="space-y-1">
                                <p className="text-xs text-muted-foreground">Còn lại</p>
                                <p className="text-base font-medium tabular-nums text-amber-600">{formatCurrency(remainingAmount)}</p>
                            </div>
                        </div>
                        ...
                        {remainingAmount > 0 ? (
                            <div className="pt-4 border-t space-y-4">
                                <div className="bg-white p-3 rounded-lg border flex flex-col items-center gap-2">
                                    <p className="text-xs text-muted-foreground">Quét QR thanh toán còn lại</p>
                                    <img src={qrUrl} alt="QR Code" className="w-40 h-40" />
                                    <p className="text-[10px] text-muted-foreground text-center leading-tight">Mã QR gen tự động theo số tiền còn lại qua SePay.</p>
                                </div>
                            </div>
                        ) : (
                            <div className="pt-4 flex items-center justify-center gap-2 text-sm text-muted-foreground">
                                <CheckCircle2 className="h-4 w-4 text-green-500" /> Đã thu hồi đủ vốn
                            </div>
                        )}
                    </CardContent>
                </Card>
                ...
                {remainingAmount > 0 && (
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-2">
                                <Label className="text-xs text-muted-foreground">Số tiền khách vừa chuyển (VNĐ)</Label>
                                <Input
                                    type="number"
